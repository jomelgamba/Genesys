...
Reuses the same .env config as the run_*.py launchers -- run this from a
folder where CLIENT_ID/CLIENT_SECRET/GENESYS_REGION are already loaded
(e.g. after `load_dotenv("configs/hcsc_pdp.env")`), or export them in the
shell first.
...
Usage:
    python cleanup_open_conversations.py --queue "HCSC PDP NONCALL Approved Enrollment"
    python cleanup_open_conversations.py --queue "HCSC PDP NONCALL Approved Enrollment" --days 3
    python cleanup_open_conversations.py --queue "HCSC PDP NONCALL Approved Enrollment" --close
"""
